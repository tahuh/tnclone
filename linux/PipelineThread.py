...
class PipelineThread(QtCore.QThread):
	...
	def _run_sorter(self):
		...
		if self.options["sort_on"] == True:
			...
			### PArallel sorting algorithm applied with fwd, rev barcode sequenes
#			TnCloneSortParallelMain(sort_conf, "AGATGTGTATAAGAGACAG", sort_path, 4) 
			...
			for line in conf_file:
				...
				if assembly_seed_error:
					...
					for line in seed_file:
						...
						ret_val = self._assemble(assembler , assem_out, KSIZE,seed_error=True)
						if ret_val == 1 :
							break
				else:
					self._assemble(assembler, assem_out, KSIZE)
	...
	def _assemble(self, assembler,assem_out, ksize, seed_error=False):
		try:
			ncontig=0
...
#				ccv_set = self.ccv_pair[sample]
	...
	def run(self):
		...
		print "[TnClone] Report files can be found at %s/report"%(self.options["out_path"])
		print "[TnClone] Assembled contigs can be found at %s/assem. Those ends with contig.final.fa"%(self.options["out_path"])
		print "[TnClone] Alignment result can be fount at %s/sam"%(self.options["out_path"])
