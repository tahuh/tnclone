...
from Stats import ContigStats , ContigStats2
...
from PvalueReporter import PvalReporter
try:
	...
	def _run_until_assembly(self):
		...
		with open(assem_conf_name) as conf_file:
					...
					try:
						...
						for path in assembler.assemble().walk():
							...
							if seq != '' :
								...
								assem_out.write(">contig:%d|sample_name:%s\n"%(ncontig,sample))
								assem_out.write(seq + "\n")
						...
						try:
							for path in assembler.assemble().walk():
								...
								if seq != '' :
									...
									#path_score = Statistics.path_score()
									#avg_depth , cv = Statistics.cv()
									#assem_out.write(">contig:%d|sample_name:%s|dp/cv=%f\n"%(ncontig, sample , avg_depth / cv))
									assem_out.write(seq + "\n")
									ncontig += 1
									check = True
						except IOError:
			...
			### sixth process : Downstream analysis
				if refpath :
					ss_find_file = outpath + "/assem/" + prefix + "_" + sample + "_k" + str(ksize) + ".contig.aln.target"
					ssf = SSF(assem_outname , ss_find_file, src, snk,self.message)
					ssf.run()
					if os.path.getsize(ss_find_file) == 0:
						if self.p_val_set.has_key(gene):
							if not self.p_val_set[gene].has_key(sample.split("_")[1]) :
								self.p_val_set[gene][sample.split("_")[1]] = []
						else:
							self.p_val_set[gene] = {}
							self.p_val_set[gene][sample.split("_")[1]] = []
						self.p_val_set[gene][sample.split("_")[1]] = 'Nan'
						continue
					### 2017-03-16 Target process -> Execute alignment
					...
					internal_align_script = src_path + "/SSW-CPP -m %d -x %d -g %d -e %d %s %s > %s"%(m,x,g,e,refname,query,samname)
					
					print "[TnClone::Caller] " + internal_align_script
					
					launch(internal_align_script)
					print "[TnClone::Pre-Alignment] Done pre-alignment..."
					...
					if len(group) >= 2 :
						Statistics = ContigStats(group,ksize,assembler.graph.bdg)
						
						Statistics.compute_path_scores()
						if self.p_val_set.has_key(gene):
							if not self.p_val_set[gene].has_key(sample.split("_")[1]) :
								self.p_val_set[gene][sample.split("_")[1]] = []
						else:
							self.p_val_set[gene] = {}
							self.p_val_set[gene][sample.split("_")[1]] = []
						if len(Statistics.seqs) > 1 :
							
							p_val = Statistics.chi_test()
							self.p_val_set[gene][sample.split("_")[1]].append(p_val)
						else:
							if len(Statistics.seqs) == 1.0:
								p_val = '-'
								self.p_val_set[gene][sample.split("_")[1]].append(p_val)
							else:
								p_val = 'NaN'
								self.p_val_set[gene][sample.split("_")[1]].append(p_val)
						seqs = Statistics.seqs
						path_scores = Statistics.path_scores
						tmp_name = outpath + "/assem/" + prefix + "_" + sample + "_k" + str(ksize) + ".contig.tmp"
						tmp_file = open(tmp_name , "w")
						print "[TnClone::FlushTmp] Flusing INDEL marked sequences"
						
						for indel_info in Statistics.indel_seqs:
							tmp_file.write(">" + indel_info[0] + "\n" + indel_info[1][1] + "\n")
						for i, normal_seq in enumerate(Statistics.seqs):
							
							ps = path_scores[i]
							avg_dp , cv = Statistics.cv(normal_seq[1][1])
							id = normal_seq[0]
							id += "|avg_depth:%d|path_score:%f|cv:%f"%(avg_dp,ps,cv)
							tmp_file.write(">" + id + "\n" + normal_seq[1][1] + "\n")
						tmp_file.close()
						print "[TnClone::DupRemoval] Selecting possible contigs"
						dup_rmv_file = outpath + "/assem/" + prefix + "_" + sample + "_k" + str(ksize) + ".contig.final.fa"
						#drt = DRT(ss_find_file , dup_rmv_file,self.message)
						drt = DRT(tmp_name , dup_rmv_file,self.message,mode=True)
						drt.run()
						assembler.clearAssembler()
						print "[TnClone::FlushTmp] Done flushing"
					else: ### No Seqs
						if self.p_val_set.has_key(gene):
							if not self.p_val_set[gene].has_key(sample.split("_")[1]) :
								self.p_val_set[gene][sample.split("_")[1]] = []
					...
					with open(ss_find_file) as f:
						for line in f:
							if not line.startswith(">"): seqs.append(line.rstrip())
							else: headers.append(line.rstrip())
							
					statistics = ContigStats(seqs,ksize,assembler.graph.bdg)
					for ppp, seq in enumerate(seqs) :
						avg_dp , cv_ = statistics.cv(seq)
						headers[ppp] += "|summary_score:%f"%(avg_dp / cv_)
						
					ss_file = open(ss_find_file , "w")
					for i in xrange(len(headers)):
						ss_file.write(headers[i] + "\n" + seqs[i] + "\n")
					ss_file.close()
					
					tmp_name = ss_find_file
					dup_rmv_file = outpath + "/assem/" + prefix + "_" + sample + "_k" + str(ksize) + ".contig.final.fa"
					#drt = DRT(ss_find_file , dup_rmv_file,self.message)
					drt = DRT(tmp_name , dup_rmv_file,self.message,mode=None)
					drt.run()
					assembler.clearAssembler()
					print "[TnClone::FlushTmp] Done flushing"
		T2 = time.time()
...
###########################################\n"""
		...
		if self.p_update:
			p_reporter = PvalReporter(self.p_val_set, report)
			p_reporter.flush()
		report.close()

		CNR = ContigNumberReporter(assem_path, report_name, pass_dict, prefix , self.p_val_set)
		KSIZE = int(self.options["ksize"])
		CNR.run(KSIZE)
		"""
